<?php
	...
	// verwerk observatie
	if (isset($_POST['id'])) {
		$result = $database->query("INSERT INTO `flora_observaties` (`soort_id`,`waarneming_id`,`datum`,`locatie`,`omschrijving`) VALUES (".intval($_POST['id']).",".intval($_POST['waarneming']).",'".$database->real_escape_string(date("Y-m-d",strtotime($_POST['datum'])))."','".$database->real_escape_string($_POST['lon']).",".$database->real_escape_string($_POST['lat'])."','".$database->real_escape_string($_POST['omschrijving'])."')");
		if (!$result) $html.= $database->error.PHP_EOL;
		
		$result = $database->query("SELECT naam_nl,naam_la,afbeelding,omschrijving,waarnemingen FROM flora WHERE id=".intval($_POST['id']));
	...
	// vul observatie in
		...
		$result = $database->query("SELECT naam_nl,naam_la,afbeelding,omschrijving,waarnemingen FROM flora WHERE id=".intval($_GET['id']));
		$table = $result->fetch_array(MYSQLI_ASSOC);
		$waarnemingen = json_decode($table["waarnemingen"]);
